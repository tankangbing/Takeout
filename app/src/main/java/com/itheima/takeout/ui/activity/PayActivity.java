...
import com.itheima.takeout.alipay.PayResult;
import com.itheima.takeout.alipay.SignUtils;
import com.itheima.takeout.model.bean.CreateOrder;
import com.itheima.takeout.model.protocol.IHttpService;
import com.itheima.takeout.presenter.PayActivityPresenter;
...
public class PayActivity extends BaseActivity {
    ...
    public void initData() {
        mPresenter = new PayActivityPresenter(this);
    }
    ...
    public void aliPay() {
        ...
        // 第2步：使用商家的私钥对orderInfo进行加密，这步操作一定要放在服务器
        /**
    ...
    public void onHttpSuccess(int reqType, Message msg) {
        // 创建订单成功，服务器返回加密后的订单信息
        if (reqType == IHttpService.TYPE_CREATE_ORDER) {
            CreateOrder bean = (CreateOrder) msg.obj;

            // 服务器返回的
            signedOrderInfo = bean.getResult();
            LogUtil.d("-----------signedOrderInfo--server: " + signedOrderInfo);

            String sign = sign(orderInfo);
            LogUtil.d("-----------signedOrderInfo--local: " + orderInfo);
            LogUtil.d("-----------signedOrderInfo--local: " + sign);

            try {
                // 仅需对sign 做URL编码
                signedOrderInfo = URLEncoder.encode(signedOrderInfo, "UTF-8");
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }

            // 第3步：通过支付宝sdk，调起支付宝应用，完成支付
            /**
             * 完整的符合支付宝参数规范的订单信息
             */
            final String payInfo = orderInfo + "&sign=\"" + signedOrderInfo + "\"&" + getSignType();
